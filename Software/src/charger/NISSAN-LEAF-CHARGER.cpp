...
void NissanLeafCharger::transmit_can(unsigned long currentMillis) {
  ...
  if (currentMillis - previousMillis10ms >= INTERVAL_10_MS) {
    ...
    if (!LEAFbatteryDetected) {
      ...
      uint16_t Vbatt = (datalayer.battery.combined.status.voltage_dV / 10) * 2;  //0-450V, 0.5V/bit
      uint16_t Ibatt = (datalayer.battery.combined.status.reported_current_dA / 10) * 2;
      ...
      LEAF_1DB.data.u8[4] = (datalayer.battery.combined.status.reported_soc / 100);  //SOC for dash
    ...
    if (PPStatus &&
      ...
      if (datalayer.battery.combined.status.voltage_dV < (datalayer.charger.charger_setpoint_HV_VDC *
                                                 10)) {  //datalayer.battery.combined.status.voltage_dV = V+1,  0-500.0 (0-5000)
        OBCpower = OBCpowerSetpoint;
      }

      // decrement charger power if volt setpoint is reached
      if (datalayer.battery.combined.status.voltage_dV >= (datalayer.charger.charger_setpoint_HV_VDC * 10)) {
        if (OBCpower > 0x64) {
          OBCpower--;
        }
      }
