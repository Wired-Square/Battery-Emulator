...
void BmwPhevBattery::HandleEquipmentStopRequest(void) {
  ...
  // they toggle the emulator's equipment-stop flag (/api/equipmentstop). Mirror that flag onto the PHEV
...
void BmwPhevBattery::update_values() {  //This function maps all the values fetched via CAN to the battery datalayer
  ...
  // PHEV balancing status values: 0=inactive not needed, 1=active, 2=not resting,
...
void BmwPhevBattery::write_advanced_status(AdvancedStatusWriter& out) {
