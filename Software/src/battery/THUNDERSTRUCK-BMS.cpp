...
void ThunderstruckBMS::
  ...
  datalayer_battery->status.soh_pptt = 9900;  //Not in CAN data, hardcoded to 99%

  datalayer_battery->status.voltage_dV = packvoltage_dV;

  datalayer_battery->status.current_dA = pack_curent_dA;

  datalayer_battery->status.remaining_capacity_Wh = static_cast<uint32_t>(
      (static_cast<double>(datalayer_battery->status.real_soc) / 10000) * datalayer_battery->info.total_capacity_Wh);

  datalayer_battery->status.max_discharge_power_W = DCLMax * (packvoltage_dV / 10);

  datalayer_battery->status.max_charge_power_W = CCLMax * (packvoltage_dV / 10);

  datalayer_battery->status.cell_max_voltage_mV = highest_cell_voltage / 10;

  datalayer_battery->status.cell_min_voltage_mV = lowest_cell_voltage / 10;
