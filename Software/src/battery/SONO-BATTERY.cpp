...
void SonoBattery::
  ...
  datalayer_battery->status.real_soc = (realSOC * 100);  //increase SOC range from 0-100 -> 100.00
  ...
  datalayer_battery->info.total_capacity_Wh = 54000;

  datalayer_battery->status.remaining_capacity_Wh = static_cast<uint32_t>(
      (static_cast<double>(datalayer_battery->status.real_soc) / 10000) * datalayer_battery->info.total_capacity_Wh);

  datalayer_battery->status.max_discharge_power_W = allowedDischargePower * 100;

  datalayer_battery->status.max_charge_power_W = allowedChargePower * 100;

  datalayer_battery->status.cell_max_voltage_mV = CellVoltMax_mV;

  datalayer_battery->status.cell_min_voltage_mV = CellVoltMin_mV;

  datalayer_battery->status.temperature_min_dC = temperatureMin;

  datalayer_battery->status.temperature_max_dC = temperatureMax;
}
