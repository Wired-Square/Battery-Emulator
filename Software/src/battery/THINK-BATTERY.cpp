...
void ThinkBattery::update_values() {
  ...
  datalayer_battery->status.real_soc = (1000 - sys_dod) * 10;

  datalayer_battery->status.soh_pptt = 9900;

  datalayer_battery->status.voltage_dV = sys_voltage;

  datalayer_battery->status.current_dA = sys_current;

  datalayer_battery->status.max_charge_power_W = (sys_voltage * sys_currentMaxCharge) / 100;

  datalayer_battery->status.max_discharge_power_W = (sys_voltage * sys_currentMaxDischarge) / 100;

  datalayer_battery->status.remaining_capacity_Wh = static_cast<uint32_t>(
      (static_cast<double>(datalayer_battery->status.real_soc) / 10000) * datalayer_battery->info.total_capacity_Wh);

  datalayer_battery->status.temperature_min_dC = (int16_t)(min_pack_temperature * 10);

  datalayer_battery->status.temperature_max_dC = (int16_t)(max_pack_temperature * 10);

  if (max_cellvoltage > 2800) {  //Value is low when unavailable
    datalayer_battery->status.cell_max_voltage_mV = max_cellvoltage;
  }

  if (min_cellvoltage > 2800) {  //Value is low when unavailable
    datalayer_battery->status.cell_min_voltage_mV = min_cellvoltage;
  }
}
