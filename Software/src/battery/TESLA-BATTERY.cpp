...
void generateTESLA_213(CAN_frame& f) {
  ...
  // The counter lives in the frame (byte 0 high nibble); reading it back
  // keeps each member frame's sequence independent across instances.
  uint8_t counter = ((f.data.u8[0] >> 4) + 1) & 0xF;
...
void TeslaBattery::
  ...
  if (type_ == BatteryType::TeslaModel3Y) {
    ...
    if (datalayer_battery->settings.user_requests_balancing) {
      static_assert(BALANCING_CELL_MAX_NCM_MV == MAX_CELL_VOLTAGE_NCA_NCM);
      static_assert(BALANCING_CELL_MAX_LFP_MV == MAX_CELL_VOLTAGE_LFP);
      static_assert(BALANCING_DEVIATION_MAX_NCM_MV == MAX_CELL_DEVIATION_NCA_NCM);
      static_assert(BALANCING_DEVIATION_MAX_LFP_MV == MAX_CELL_DEVIATION_LFP);
      static_assert(BALANCING_PACK_MAX_NCM_DV == MAX_PACK_VOLTAGE_3Y_NCMA);
      static_assert(BALANCING_PACK_MAX_LFP_DV == MAX_PACK_VOLTAGE_3Y_LFP);
      // Clamp against the chemistry limits applied just above, not the web
      // layer's bounds: a setting accepted before autodetect flipped the
      // chemistry to LFP must not raise the LFP trip points.
      datalayer_battery->info.max_design_voltage_dV =
          std::min<uint16_t>(datalayer_battery->settings.balancing_max_pack_voltage_dV,
                             datalayer_battery->info.max_design_voltage_dV + BALANCING_PACK_HEADROOM_DV);
      datalayer_battery->info.max_cell_voltage_mV = std::min(
          datalayer_battery->settings.balancing_max_cell_voltage_mV, datalayer_battery->info.max_cell_voltage_mV);
      datalayer_battery->info.max_cell_voltage_deviation_mV =
          std::min(datalayer_battery->settings.balancing_max_deviation_cell_voltage_mV,
                   datalayer_battery->info.max_cell_voltage_deviation_mV);
      datalayer_battery->status.max_charge_power_W =
          std::min<uint32_t>(datalayer_battery->settings.balancing_float_power_W, BALANCING_FLOAT_POWER_MAX_W);
  ...
  if (parsed_battery_serialNumber && battery_serialNumber[13] != 0) {
    ...
    if (battery_manufactureDate) {
      snprintf(datalayer_tesla->battery_manufactureDate,
               sizeof(datalayer_tesla->battery_manufactureDate), "%s", battery_manufactureDate);
    }
  ...
  datalayer_tesla->battery_platformMaxBusVoltage = battery_platformMaxBusVoltage;
  //0x2D2
  ...
  datalayer_tesla->HVP_gpioPackCoontPosFlywheel = HVP_gpioPackCoontPosFlywheel;
  datalayer_tesla->HVP_gpioCpLatchEnable = HVP_gpioCpLatchEnable;
  datalayer_tesla->HVP_gpioPcsEnable = HVP_gpioPcsEnable;
  datalayer_tesla->HVP_gpioPcsDcdcPwmEnable = HVP_gpioPcsDcdcPwmEnable;
  datalayer_tesla->HVP_gpioPcsChargePwmEnable = HVP_gpioPcsChargePwmEnable;
  datalayer_tesla->HVP_gpioFcContPowerEnable = HVP_gpioFcContPowerEnable;
  datalayer_tesla->HVP_gpioHvilEnable = HVP_gpioHvilEnable;
  datalayer_tesla->HVP_gpioSecDrdy = HVP_gpioSecDrdy;
  ...
  datalayer_tesla->HVP_shuntRefVoltageMismatch = HVP_shuntRefVoltageMismatch;
  datalayer_tesla->HVP_shuntThermistorMia = HVP_shuntThermistorMia;
  datalayer_tesla->HVP_shuntHwMia = HVP_shuntHwMia;
...
void TeslaBattery::write_advanced_status(AdvancedStatusWriter& out) {
