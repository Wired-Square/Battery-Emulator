...
void GrowattHvInverter::
  ...
  // BMS that provides the actual series cell count in datalayer.battery.combined.info.number_of_cells (e.g. 119).
  ...
  if (datalayer.battery.combined.info.number_of_cells >= 10) {
    total_cells = (uint16_t)datalayer.battery.combined.info.number_of_cells;
  }
  ...
  if (datalayer.battery.combined.status.voltage_dV > 10) {  // Only update value when we have voltage available to avoid div0
    ...
    const uint16_t v_dV = datalayer.battery.combined.status.voltage_dV;

    uint32_t full_10mAh = (uint32_t)((uint64_t)datalayer.battery.combined.info.reported_total_capacity_Wh * 1000ULL / v_dV);

    uint32_t rem_10mAh = 0;
    if (datalayer.battery.combined.status.reported_remaining_capacity_Wh > 0) {
      rem_10mAh = (uint32_t)((uint64_t)datalayer.battery.combined.status.reported_remaining_capacity_Wh * 1000ULL / v_dV);
    } else {
      // Fallback: derive remaining capacity from SOC if remaining Wh is not available.
      const uint32_t soc_pct = (uint32_t)(datalayer.battery.combined.status.reported_soc / 100);  // 0..100
  ...
  if (datalayer.battery.settings.user_set_voltage_limits_active) {  //If user is requesting a specific voltage
    ...
    GROWATT_3110.data.u8[0] = (datalayer.battery.combined.info.max_design_voltage_dV >> 8);
    GROWATT_3110.data.u8[1] = (datalayer.battery.combined.info.max_design_voltage_dV & 0x00FF);
  }
  //Charge limited current, 125 =12.5A (0.1, A) (Min 0, Max 300A)
  GROWATT_3110.data.u8[2] = (datalayer.battery.combined.status.max_charge_current_dA >> 8);
  GROWATT_3110.data.u8[3] = (datalayer.battery.combined.status.max_charge_current_dA & 0x00FF);
  //Discharge limited current, 500 = 50A, (0.1, A)
  GROWATT_3110.data.u8[4] = (datalayer.battery.combined.status.max_discharge_current_dA >> 8);
  GROWATT_3110.data.u8[5] = (datalayer.battery.combined.status.max_discharge_current_dA & 0x00FF);
  ...
  if ((datalayer.battery.combined.status.max_discharge_current_dA == 0) || (datalayer.battery.combined.status.reported_soc == 0) ||
      (datalayer.system.status.system_status == FAULT)) {
  ...
  GROWATT_3130.data.u8[0] = (datalayer.battery.combined.status.voltage_dV >> 8);
  GROWATT_3130.data.u8[1] = (datalayer.battery.combined.status.voltage_dV & 0x00FF);
  //Total current (0.1A -300 to 300A)
  GROWATT_3130.data.u8[2] = (datalayer.battery.combined.status.reported_current_dA >> 8);
  GROWATT_3130.data.u8[3] = (datalayer.battery.combined.status.reported_current_dA & 0x00FF);
  //Cell max temperature (0.1C) [-40 to 120*C]
  GROWATT_3130.data.u8[4] = (datalayer.battery.combined.status.temperature_max_dC >> 8);
  GROWATT_3130.data.u8[5] = (datalayer.battery.combined.status.temperature_max_dC & 0x00FF);
  //SOC (%) [0-100]
  GROWATT_3130.data.u8[6] = (datalayer.battery.combined.status.reported_soc / 100);
  //SOH (%) (Bit 0~ Bit6 SOH Counters) Bit7 SOH flag (Indicates that battery is in unsafe use)
  GROWATT_3130.data.u8[7] = (datalayer.battery.combined.status.soh_pptt / 100);
  ...
  if (datalayer.battery.settings.user_set_voltage_limits_active) {  //If user is requesting a specific voltage
    ...
    GROWATT_3150.data.u8[0] = (datalayer.battery.combined.info.min_design_voltage_dV >> 8);
    GROWATT_3150.data.u8[1] = (datalayer.battery.combined.info.min_design_voltage_dV & 0x00FF);
  }
  //Main control unit temperature (0.1C) [-40 to 120*C]
  GROWATT_3150.data.u8[2] = (datalayer.battery.combined.status.temperature_max_dC >> 8);
  GROWATT_3150.data.u8[3] = (datalayer.battery.combined.status.temperature_max_dC & 0x00FF);
  ...
  GROWATT_3160.data.u8[6] = (datalayer.battery.combined.status.temperature_min_dC >> 8);
  GROWATT_3160.data.u8[7] = (datalayer.battery.combined.status.temperature_min_dC & 0x00FF);
  ...
  GROWATT_3190.data.u8[1] = (datalayer.battery.combined.status.cell_max_voltage_mV >> 8);
  GROWATT_3190.data.u8[2] = (datalayer.battery.combined.status.cell_max_voltage_mV & 0x00FF);
  // Min cell voltage (mV)
  GROWATT_3190.data.u8[3] = (datalayer.battery.combined.status.cell_min_voltage_mV >> 8);
  GROWATT_3190.data.u8[4] = (datalayer.battery.combined.status.cell_min_voltage_mV & 0x00FF);
  //Reserved
  ...
  GROWATT_3200.data.u8[6] = (datalayer.battery.combined.info.max_cell_voltage_mV >> 8);
  GROWATT_3200.data.u8[7] = (datalayer.battery.combined.info.max_cell_voltage_mV & 0x00FF);
  ...
  GROWATT_3220.data.u8[5] = ((datalayer.battery.combined.info.reported_total_capacity_Wh / 100) >> 8);
  GROWATT_3220.data.u8[6] = ((datalayer.battery.combined.info.reported_total_capacity_Wh / 100) & 0x00FF);
