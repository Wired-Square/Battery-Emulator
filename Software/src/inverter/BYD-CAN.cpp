...
void BydCanInverter::
  ...
  uint16_t nominal_voltage_dV =
      (datalayer.battery.combined.info.max_design_voltage_dV + datalayer.battery.combined.info.min_design_voltage_dV) / 2;
  if (nominal_voltage_dV < 100) {
    nominal_voltage_dV = datalayer.battery.combined.status.voltage_dV;
  }
  if (nominal_voltage_dV > 10) {
    remaining_capacity_ah = (datalayer.battery.combined.status.reported_remaining_capacity_Wh * 100UL) / nominal_voltage_dV;
    fully_charged_capacity_ah = (datalayer.battery.combined.info.reported_total_capacity_Wh * 100UL) / nominal_voltage_dV;
  }
  ...
  if (datalayer.battery.settings.user_set_voltage_limits_active) {  //If user is requesting a specific voltage
    ...
    //Target charge voltage (eg 400.0V = 4000 , 16bits long)
    BYD_110.data.u8[0] = ((datalayer.battery.combined.info.max_design_voltage_dV - VOLTAGE_OFFSET_DV) >> 8);
    BYD_110.data.u8[1] = ((datalayer.battery.combined.info.max_design_voltage_dV - VOLTAGE_OFFSET_DV) & 0x00FF);
    //Target discharge voltage (eg 300.0V = 3000 , 16bits long)
    BYD_110.data.u8[2] = ((datalayer.battery.combined.info.min_design_voltage_dV + VOLTAGE_OFFSET_DV) >> 8);
    BYD_110.data.u8[3] = ((datalayer.battery.combined.info.min_design_voltage_dV + VOLTAGE_OFFSET_DV) & 0x00FF);
  }

  //Maximum discharge power allowed (Unit: A+1)
  BYD_110.data.u8[4] = (datalayer.battery.combined.status.max_discharge_current_dA >> 8);
  BYD_110.data.u8[5] = (datalayer.battery.combined.status.max_discharge_current_dA & 0x00FF);
  //Maximum charge power allowed (Unit: A+1)
  BYD_110.data.u8[6] = (datalayer.battery.combined.status.max_charge_current_dA >> 8);
  BYD_110.data.u8[7] = (datalayer.battery.combined.status.max_charge_current_dA & 0x00FF);

  //SOC (100.00%)
  BYD_150.data.u8[0] = (datalayer.battery.combined.status.reported_soc >> 8);
  BYD_150.data.u8[1] = (datalayer.battery.combined.status.reported_soc & 0x00FF);
  ...
  BYD_150.data.u8[2] = (datalayer.battery.combined.status.soh_pptt >> 8);
  BYD_150.data.u8[3] = (datalayer.battery.combined.status.soh_pptt & 0x00FF);
  ...
  //Voltage (ex 370.0)
  BYD_1D0.data.u8[0] = (datalayer.battery.combined.status.voltage_dV >> 8);
  BYD_1D0.data.u8[1] = (datalayer.battery.combined.status.voltage_dV & 0x00FF);
  //Current (ex 81.0A)
  BYD_1D0.data.u8[2] = (datalayer.battery.combined.status.reported_current_dA >> 8);
  BYD_1D0.data.u8[3] = (datalayer.battery.combined.status.reported_current_dA & 0x00FF);
  //Temperature average
  BYD_1D0.data.u8[4] = (temperature_average >> 8);
  BYD_1D0.data.u8[5] = (temperature_average & 0x00FF);

  //Temperature max
  BYD_210.data.u8[0] = (datalayer.battery.combined.status.temperature_max_dC >> 8);
  BYD_210.data.u8[1] = (datalayer.battery.combined.status.temperature_max_dC & 0x00FF);
  //Temperature min
  BYD_210.data.u8[2] = (datalayer.battery.combined.status.temperature_min_dC >> 8);
  BYD_210.data.u8[3] = (datalayer.battery.combined.status.temperature_min_dC & 0x00FF);
  //Capacity
  BYD_250.data.u8[4] = (uint8_t)((datalayer.battery.combined.info.reported_total_capacity_Wh / 100) >> 8);
  BYD_250.data.u8[5] = (uint8_t)(datalayer.battery.combined.info.reported_total_capacity_Wh / 100);
}
