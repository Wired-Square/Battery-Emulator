...
void FerroampCanInverter::
  ...
  //SOC (100.00%)
  FERROAMP_4211.data.u8[6] = (datalayer.battery.combined.status.reported_soc / 100);  //Remove decimals

  //StateOfHealth (100.00%)
  FERROAMP_4211.data.u8[7] = (datalayer.battery.combined.status.soh_pptt / 100);
  ...
  //Voltage (370.0)
  FERROAMP_4211.data.u8[0] = (datalayer.battery.combined.status.voltage_dV & 0x00FF);
  FERROAMP_4211.data.u8[1] = (datalayer.battery.combined.status.voltage_dV >> 8);

  //Current (15.0)
  FERROAMP_4211.data.u8[2] = ((datalayer.battery.combined.status.reported_current_dA + 30000) & 0x00FF);
  FERROAMP_4211.data.u8[3] = ((datalayer.battery.combined.status.reported_current_dA + 30000) >> 8);

  // BMS Temperature (We dont have BMS temp, send max cell voltage instead)
  FERROAMP_4211.data.u8[4] = ((datalayer.battery.combined.status.temperature_max_dC + 1000) & 0x00FF);
  FERROAMP_4211.data.u8[5] = ((datalayer.battery.combined.status.temperature_max_dC + 1000) >> 8);

  //Maxvoltage (eg 400.0V = 4000 , 16bits long) Discharge Cutoff Voltage
  FERROAMP_4221.data.u8[0] = (datalayer.battery.combined.info.max_design_voltage_dV & 0x00FF);
  FERROAMP_4221.data.u8[1] = (datalayer.battery.combined.info.max_design_voltage_dV >> 8);

  //Minvoltage (eg 300.0V = 3000 , 16bits long) Charge Cutoff Voltage
  FERROAMP_4221.data.u8[2] = (datalayer.battery.combined.info.min_design_voltage_dV & 0x00FF);
  FERROAMP_4221.data.u8[3] = (datalayer.battery.combined.info.min_design_voltage_dV >> 8);

  //Max ChargeCurrent
  FERROAMP_4221.data.u8[4] = ((datalayer.battery.combined.status.max_charge_current_dA + 30000) & 0x00FF);
  FERROAMP_4221.data.u8[5] = ((datalayer.battery.combined.status.max_charge_current_dA + 30000) >> 8);

  //Max DischargeCurrent
  FERROAMP_4221.data.u8[6] = ((30000 - datalayer.battery.combined.status.max_discharge_current_dA) & 0x00FF);
  FERROAMP_4221.data.u8[7] = ((30000 - datalayer.battery.combined.status.max_discharge_current_dA) >> 8);
  ...
  FERROAMP_4241.data.u8[0] = (datalayer.battery.combined.status.temperature_max_dC & 0x00FF);
  FERROAMP_4241.data.u8[1] = (datalayer.battery.combined.status.temperature_max_dC >> 8);

  //Min temperature per cell
  FERROAMP_4241.data.u8[2] = (datalayer.battery.combined.status.temperature_min_dC & 0x00FF);
  FERROAMP_4241.data.u8[3] = (datalayer.battery.combined.status.temperature_min_dC >> 8);

  //Max temperature per module
  FERROAMP_4271.data.u8[0] = (datalayer.battery.combined.status.temperature_max_dC & 0x00FF);
  FERROAMP_4271.data.u8[1] = (datalayer.battery.combined.status.temperature_max_dC >> 8);

  //Min temperature per module
  FERROAMP_4271.data.u8[2] = (datalayer.battery.combined.status.temperature_min_dC & 0x00FF);
  FERROAMP_4271.data.u8[3] = (datalayer.battery.combined.status.temperature_min_dC >> 8);
