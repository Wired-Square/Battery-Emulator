...
void GrowattWitInverter::update_values() {
  ...
  GROWATT_1AC3.data.u8[0] = (datalayer.battery.combined.status.max_charge_current_dA & 0xFF);
  GROWATT_1AC3.data.u8[1] = (datalayer.battery.combined.status.max_charge_current_dA >> 8);

  // Byte 2-3: Max Discharge Current (0.1A, 0-10000)
  GROWATT_1AC3.data.u8[2] = (datalayer.battery.combined.status.max_discharge_current_dA & 0xFF);
  GROWATT_1AC3.data.u8[3] = (datalayer.battery.combined.status.max_discharge_current_dA >> 8);
  ...
  if (datalayer.battery.settings.user_set_voltage_limits_active) {
    ...
    max_charge_voltage_dV = datalayer.battery.combined.info.max_design_voltage_dV;
  }
  ...
  if (datalayer.battery.settings.user_set_voltage_limits_active) {
    ...
    min_discharge_voltage_dV = datalayer.battery.combined.info.min_design_voltage_dV;
  }
  ...
  } else if (datalayer.battery.combined.status.reported_current_dA > 5) {
  ...
  if (datalayer.battery.combined.status.max_charge_current_dA == 0 || datalayer.battery.combined.status.reported_soc >= 10000 ||  // 100%
  ...
  // Byte 0: SOC (0-100%)
  GROWATT_1AC6.data.u8[0] = (uint8_t)(datalayer.battery.combined.status.reported_soc / 100);

  // Byte 1: SOH (0-100%, bit7 = scrap warning if 1)
  uint8_t soh = (uint8_t)(datalayer.battery.combined.status.soh_pptt / 100);
  if (soh < 50) {
  ...
  if (datalayer.battery.combined.status.voltage_dV > 0) {
    ...
    uint32_t capacity_calc =
        (datalayer.battery.combined.info.reported_total_capacity_Wh * 100UL) / datalayer.battery.combined.status.voltage_dV;
  ...
  GROWATT_1AC7.data.u8[2] = (datalayer.battery.combined.status.voltage_dV & 0xFF);
  GROWATT_1AC7.data.u8[3] = (datalayer.battery.combined.status.voltage_dV >> 8);

  // Byte 4-5: Battery Current (0.1A, offset -1000A)
  // Raw = (Actual + 1000) * 10 = Actual_dA + 10000
  int16_t current_dA = datalayer.battery.combined.status.reported_current_dA;
  ...
  uint8_t cells_per_module = datalayer.battery.combined.info.number_of_cells;
  ...
  uint32_t voltage_cV_calc = (uint32_t)datalayer.battery.combined.status.voltage_dV * 10;
  ...
  int16_t temp_max_dC = datalayer.battery.combined.status.temperature_max_dC;
  ...
  int16_t temp_avg_dC = (datalayer.battery.combined.status.temperature_max_dC + datalayer.battery.combined.status.temperature_min_dC) / 2;
  ...
  int16_t temp_min_dC = datalayer.battery.combined.status.temperature_min_dC;
  ...
  uint16_t cell_max_mV = datalayer.battery.combined.status.cell_max_voltage_mV;
  ...
  uint16_t cell_avg_mV =
      (datalayer.battery.combined.status.cell_max_voltage_mV + datalayer.battery.combined.status.cell_min_voltage_mV) / 2;
  ...
  uint16_t cell_min_mV = datalayer.battery.combined.status.cell_min_voltage_mV;
  ...
  uint8_t soc_percent = (uint8_t)(datalayer.battery.combined.status.reported_soc / 100);
  ...
  if (datalayer.battery.combined.status.total_charged_battery_Wh > 0) {
    acc_charge_01kWh = (uint32_t)(datalayer.battery.combined.status.total_charged_battery_Wh / 100);
  }
  ...
  if (datalayer.battery.combined.status.total_discharged_battery_Wh > 0) {
    acc_discharge_01kWh = (uint32_t)(datalayer.battery.combined.status.total_discharged_battery_Wh / 100);
  }
  ...
  int32_t power_W = datalayer.battery.combined.status.active_power_W;
  ...
  if (datalayer.battery.combined.status.cell_max_voltage_mV >= datalayer.battery.combined.info.max_cell_voltage_mV) {
    charge_alarm |= 0x01;       // Cell overvoltage alarm
    charge_protection |= 0x01;  // Cell overvoltage protection
  }
  if (datalayer.battery.combined.status.cell_min_voltage_mV <= datalayer.battery.combined.info.min_cell_voltage_mV) {
    discharge_alarm |= 0x01;       // Cell undervoltage alarm
    discharge_protection |= 0x01;  // Cell undervoltage protection
  }

  // Check temperature limits (assuming reasonable limits)
  if (datalayer.battery.combined.status.temperature_max_dC > 550) {  // > 55°C
  ...
  if (datalayer.battery.combined.status.temperature_min_dC < 0) {  // < 0°C
  ...
  uint16_t cell_deviation = datalayer.battery.combined.status.cell_max_voltage_mV - datalayer.battery.combined.status.cell_min_voltage_mV;
  if (cell_deviation > datalayer.battery.combined.info.max_cell_voltage_deviation_mV) {
    generic_alarm |= 0x01;       // Cell voltage difference excessive
    generic_protection |= 0x01;  // Cell voltage difference protection
  }

  // Check temperature difference
  int16_t temp_diff = datalayer.battery.combined.status.temperature_max_dC - datalayer.battery.combined.status.temperature_min_dC;
