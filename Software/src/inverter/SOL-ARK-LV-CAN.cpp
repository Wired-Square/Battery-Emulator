...
void SolArkLvInverter::update_values() {
  ...
  SOLARK_351.data.u8[2] = datalayer.battery.combined.status.max_charge_current_dA & 0xff;
  SOLARK_351.data.u8[3] = datalayer.battery.combined.status.max_charge_current_dA >> 8;
  SOLARK_351.data.u8[4] = datalayer.battery.combined.status.max_discharge_current_dA & 0xff;
  SOLARK_351.data.u8[5] = datalayer.battery.combined.status.max_discharge_current_dA >> 8;
  SOLARK_351.data.u8[6] = datalayer.battery.combined.info.min_design_voltage_dV & 0xff;
  SOLARK_351.data.u8[7] = datalayer.battery.combined.info.min_design_voltage_dV >> 8;

  SOLARK_355.data.u8[0] = (datalayer.battery.combined.status.reported_soc / 100) & 0xff;
  SOLARK_355.data.u8[1] = (datalayer.battery.combined.status.reported_soc / 100) >> 8;
  SOLARK_355.data.u8[2] = (datalayer.battery.combined.status.soh_pptt / 100) & 0xff;
  SOLARK_355.data.u8[3] = (datalayer.battery.combined.status.soh_pptt / 100) >> 8;

  int16_t average_temperature =
      (datalayer.battery.combined.status.temperature_min_dC + datalayer.battery.combined.status.temperature_max_dC) / 2;
  SOLARK_356.data.u8[0] = datalayer.battery.combined.status.voltage_dV & 0xff;
  SOLARK_356.data.u8[1] = datalayer.battery.combined.status.voltage_dV >> 8;
  SOLARK_356.data.u8[2] = datalayer.battery.combined.status.reported_current_dA & 0xff;
  SOLARK_356.data.u8[3] = datalayer.battery.combined.status.reported_current_dA >> 8;
  ...
  if (datalayer.battery.combined.status.reported_current_dA >= (datalayer.battery.combined.status.max_discharge_current_dA + 50))
