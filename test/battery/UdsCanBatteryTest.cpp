...
TEST_F(UdsCanBatteryTest, PidScanRequestsPidsInOrderAndWraps) {
  ...
  ASSERT_EQ(recorded_frames().size(), 1u);
  EXPECT_EQ(last_frame(recorded_frames()).ID, 0x79Bu);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[0], 0x03);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[1], 0x22);
  EXPECT_EQ(requested_pid(last_frame(recorded_frames())), 0x9001);
  ...
  ASSERT_EQ(recorded_frames().size(), 2u);
  EXPECT_EQ(requested_pid(last_frame(recorded_frames())), 0x9002);

  feed_response({0x62, 0x90, 0x02, 0x00, 0x64});
  tick(1200);
  // Check the next request is for 0x9003.
  EXPECT_EQ(requested_pid(last_frame(recorded_frames())), 0x9003);
...
TEST_F(UdsCanBatteryTest, PidScanIsIdleWithoutAList) {
  ...
  EXPECT_TRUE(recorded_frames().empty());
...
TEST_F(UdsCanBatteryTest, DetourPidIsRequestedBeforeScanResumes) {
  ...
  tick(1100);
  EXPECT_EQ(requested_pid(last_frame(recorded_frames())), 0x9007);
...
TEST_F(UdsCanBatteryTest, ResponsePendingKeepsWaitingForTheRealResponse) {
  ...
  tick(1100);
  EXPECT_EQ(recorded_frames().size(), 1u);
...
TEST_F(UdsCanBatteryTest, LateResponseForAnotherPidIsNotConsumed) {
  ...
  EXPECT_EQ(requested_pid(last_frame(recorded_frames())), 0x9002);  // 0x9002 is now pending.
  EXPECT_TRUE(battery->pid_calls.empty());
...
TEST_F(UdsCanBatteryTest, SequenceStepResponseIsDispatchedToTheSubclass) {
  ...
  ASSERT_EQ(recorded_frames().size(), 1u);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[0], 0x02);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[1], 0x11);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[2], 0x01);
...
TEST_F(UdsCanBatteryTest, SequenceStepRetriesThenTimesOut) {
  ...
  EXPECT_EQ(recorded_frames().size(), 2u);

  tick(1200);  // busy
  tick(1300);  // retry 2.
  EXPECT_EQ(recorded_frames().size(), 3u);
...
TEST_F(UdsCanBatteryTest, MultiFrameSequenceRequestIsSent) {
  ...
  ASSERT_EQ(recorded_frames().size(), 1u);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[0], 0x10);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[1], 0x0B);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[2], 0x2E);  // WriteDataByIdentifier
  ...
  tick(1000);
  ASSERT_EQ(recorded_frames().size(), 2u);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[0], 0x21);  // CF, SN=1
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[1], 0x06);  // First remaining payload byte
...
TEST_F(UdsCanBatteryTest, SequenceHoldsTheBusFromPidScan) {
  ...
  ASSERT_EQ(battery->seq_calls.size(), 1u);
  EXPECT_EQ(recorded_frames().size(), 0u);
...
TEST_F(UdsCanBatteryTest, SequenceStartedDuringPauseIsHeldUntilPauseExpires) {
  ...
  ASSERT_EQ(recorded_frames().size(), 1u);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[1], 0x19);
...
TEST_F(UdsCanBatteryTest, ExternalToolTrafficBacksOff) {
  ...
  tick(1000);
  EXPECT_TRUE(recorded_frames().empty());  // 5 s backoff (50 ticks).
  EXPECT_TRUE(battery->uds_is_busy());
...
TEST_F(UdsCanBatteryTest, ReadDtcParsesSingleFrameResponse) {
  ...
  ASSERT_EQ(recorded_frames().size(), 1u);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[1], 0x19);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[2], 0x02);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[3], 0x09);
  ...
  EXPECT_EQ(datalayer.battery.pack[0].dtc.dtc_codes[0], 0xC19B00u);
  EXPECT_EQ(datalayer.battery.pack[0].dtc.dtc_status[0], 0xFF);
...
TEST_F(UdsCanBatteryTest, ClearDtcSequenceCompletesOnAcknowledgment) {
  ...
  ASSERT_EQ(recorded_frames().size(), 1u);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[1], 0x14);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[2], 0xFF);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[3], 0xFF);
  EXPECT_EQ(last_frame(recorded_frames()).data.u8[4], 0xFF);
  EXPECT_TRUE(battery->uds_is_busy());
