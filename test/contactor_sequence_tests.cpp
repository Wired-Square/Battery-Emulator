...
TEST_F(ContactorSequenceTest, DoesNotCloseContactorsDuringTheStartupWindow) {
  ...
  for (unsigned long t = 0; t < INTERVAL_10_S; t += 1000) {
    tick_at(t);
    ASSERT_EQ(contactorStatus, ContactorState::START_PRECHARGE) << "the ladder advanced at t=" << t << ", inside the startup window";
  }

  tick_at(INTERVAL_10_S + 1);
  EXPECT_EQ(contactorStatus, ContactorState::PRECHARGE) << "the ladder must start once the window has passed";
}
...
TEST_F(ContactorSequenceTest, DoesNotCloseContactorsBeforeBatteryIsDetected) {
  ...
  EXPECT_EQ(contactorStatus, ContactorState::START_PRECHARGE);
}
...
TEST_F(ContactorSequenceTest, LeavesDisconnectedOncePermittedAndNoStop) {
  ...
  EXPECT_EQ(contactorStatus, ContactorState::PRECHARGE);
...
TEST_F(ContactorSequenceTest, ClosesInOrderWithEachStepGatedOnItsTimer) {
  ...
  ASSERT_EQ(contactorStatus, ContactorState::PRECHARGE);
  EXPECT_EQ(datalayer.system.status.contactors_engaged, kEngagedClosing);

  // Too early for precharge.
  tick_at(negative_closed_at + kNegativeToPrechargeMs - 1);
  EXPECT_EQ(contactorStatus, ContactorState::PRECHARGE) << "precharge engaged before the negative settling time elapsed";

  tick_at(negative_closed_at + kNegativeToPrechargeMs);
  ASSERT_EQ(contactorStatus, ContactorState::POSITIVE);
  ...
  EXPECT_EQ(contactorStatus, ContactorState::POSITIVE) << "positive contactor closed before precharge finished";

  tick_at(precharge_started_at + precharge_time_ms);
  ASSERT_EQ(contactorStatus, ContactorState::PRECHARGE_OFF);
  ...
  EXPECT_EQ(contactorStatus, ContactorState::PRECHARGE_OFF);
...
TEST_F(ContactorSequenceTest, DcBusIsLiveOnlyWhenTheSequenceHasCompleted) {
  ...
  ASSERT_EQ(contactorStatus, ContactorState::COMPLETED);
...
TEST_F(ContactorSequenceTest, SustainedFaultLatchesShutdownAndDoesNotRecover) {
  ...
  ASSERT_EQ(contactorStatus, ContactorState::SHUTDOWN_REQUESTED);
  ...
  EXPECT_EQ(contactorStatus, ContactorState::SHUTDOWN_REQUESTED) << "the fault latch must survive the fault clearing";
  EXPECT_EQ(datalayer.system.status.contactors_engaged, kEngagedFaultLatched);
}
...
TEST_F(ContactorSequenceTest, BriefFaultDoesNotLatchShutdown) {
  ...
  EXPECT_EQ(contactorStatus, ContactorState::COMPLETED) << "two short faults must not add up to a latch";
}
...
TEST_F(ContactorSequenceTest, ExactlyTheAllowedFaultTicksDoesNotLatch) {
  ...
  EXPECT_EQ(contactorStatus, ContactorState::COMPLETED) << "latched at the threshold instead of beyond it";
}
...
TEST_F(ContactorSequenceTest, ShutdownLatchRaisesTheOpenContactorEvent) {
  ...
  for (unsigned long i = 0; i <= kFaultTicksBeforeShutdown + 1 && contactorStatus != ContactorState::SHUTDOWN_REQUESTED; ++i) {
    tick_at(kBootMs + i);
  }
  ASSERT_EQ(contactorStatus, ContactorState::SHUTDOWN_REQUESTED);
